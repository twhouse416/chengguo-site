...
在 R.TUBE、Facebook、Word 等地方寫好的文章，可以直接複製貼過來：

1. 到來源網站全選複製整篇文章
2. 後台 → 文章 → 編輯（或新增）→ 展開「貼上長文，自動分段」
3. 貼上 → 按「取代全部內容」

系統會自動整理成段落、小標、條列，並順便抓出：
...
| 摘要 | 第一個夠長的段落，前 90 字 |
| 關鍵字 | 文中的 #hashtag |
| Q&A 問答 | `Q：` `A：` 開頭的段落會收進 FAQ |

平台的介面文字（發佈時間、分享、留言、上一篇等）會自動略過。
已經填過的標題與摘要不會被覆蓋。

**貼完之後建議做的事**：改一個更好搜尋的標題、補幾題 FAQ、確認關鍵字。
這樣你網站的版本會比來源更完整，不是單純的複製。

### 從網址自動匯入（部分網站可用）

`data/import-queue.json` 的 `urls` 填入文章網址並推上 GitHub，
Actions 會自動抓回內容存成草稿。

**但有些網站會拒絕自動讀取**（例如 R.TUBE 有 Cloudflare 防護，會回 403）。
那種情況請改用上面的複製貼上方式。
...
`data/site-config.json` 的 `rtubeChannel` 設為你的 R.TUBE 頻道代號後，
每天早上 6:00 會自動把新文章同步進本站。

### 運作方式

- 只抓你自己頻道的文章（這些是澄果團隊自己寫的，著作權在自己手上）
- 標題、摘要、封面圖、發布日期、內文段落、條列、表格都會轉成本站格式
- **同步進來的一律是草稿**，要到後台確認內容與排版後才發布
- **已存在的文章不會被覆蓋**，你在後台改過的內容會保留

### ⚠️ 兩件要注意的事

**1. 本站是原版，R.TUBE 是曝光管道**

本站文章的 canonical 指向本站，這樣 Google 才會把權重算給你的網站。
如果反過來讓 R.TUBE 當原版，你的網站會被判定為複製品，拿不到排名。

在 R.TUBE 發文時，建議在文末加一句「原文出處：<你的文章網址>」。

**2. R.TUBE 頻道要設為公開**

若頻道是非公開狀態，訪客與搜尋引擎看不到那邊的文章，同步腳本執行時會顯示警告。

### 手動執行

Actions →「同步 R.TUBE 文章」→ Run workflow。

本機測試可用 `node scripts/fetch-rtube.js --dry`，只顯示會抓到什麼、不寫檔。
...
在後台的「賀成交」分頁新增。**首頁顯示最新 6 筆**，全部紀錄在 `/deals/` 獨立頁面，依區域分組。

新增時填：圖片、區域、社區名稱（可留空）、說明。圖片會自動壓縮上傳到 `assets/deals/`，並依原圖比例完整顯示，不會被裁切。
...
在 YouTube 把影片改成「不公開」「私人」或刪除後，下次同步時網站會**自動隱藏**它，
後台會標示「已在 YouTube 下架」並記錄日期。

不想等 6 小時的話，可以在後台手動勾選「在網站上隱藏這支影片」，或到 Actions 手動執行同步。

判斷方式：YouTube 的 RSS 只提供最新 15 支，所以程式用「RSS 中最舊那支的發布日」當界線——
比它新卻沒出現在 RSS 裡的，才判定為已下架。真正只是舊而滑出 15 支範圍的影片不受影響。
